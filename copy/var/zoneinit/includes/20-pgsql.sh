...
log "create user and database for netbox"
if ! psql -U postgres -lqt | cut -d \| -f 1 | grep -qw netbox 2> /dev/null; then
    export PGPASSWORD=$(mdata-get pgsql_pw)
    createuser -U postgres -s netbox
    createdb netbox -U postgres -O netbox \
        --encoding=UTF8 --locale=C --template=template0

    if USER_PGSQL_PW=$(/opt/core/bin/mdata-create-password.sh -m netbox_pgsql_pw 2> /dev/null); then
        psql -U netbox -d netbox -c "alter user postgres with password '${USER_PGSQL_PW}';"
    fi
fi
